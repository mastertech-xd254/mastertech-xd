...
setInterval(async () => {
    if (conf.AUTO_BIO === "yes") {
        ...
        const bioText = `👻 MASTERTECH-MD 👻 ɪs ᴏɴʟɪɴᴇ : ${currentDateTime}`; // Format the bio text
    ...
    zk.ev.on("messages.upsert", async (m) => {
        ...
        zk.ev.on("messages.upsert", async (m) => {
            ...
            /*  var superUser=[servBot,dj,dj2,luffy].map((s)=>s.replace(/[^0-9]/g)+"@s.whatsapp.net").includes(auteurMessage);
              var dev =[masterpeace].map((t)=>t.replace(/[^0-9]/g)+"@s.whatsapp.net").includes(auteurMessage);*/
            ...
            const superUserNumbers = [servBot, masterpeace, conf.NUMERO_OWNER].map((s) => s.replace(/[^0-9]/g) + "@s.whatsapp.net");
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                ...
                fs.readdirSync(__dirname + "/commandes").forEach((fichier) => {
                    if (path.extname(fichier).toLowerCase() == (".js")) {
                        try {
                            ...
                            console.log(fichier + " Loaded successfully by mastertech-md✔️");
                        }
                        catch (e) {
                ...
                let cmsg =` mastertech-ᴍᴅ ʙᴏᴛ ɪs ᴄᴏɴɴᴇᴄᴛᴇᴅ...
